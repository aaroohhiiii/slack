...
const {data:users = [],isLoading , isError} = useQuery({
    queryKey :["usersList" , client?.user?.id],
    queryFn : fetchUsers,
    enabled : !!client.user,
...
/* staletime is tell react query that data is fresh for 5 mins during these 5 mins react eont fetch data automativllly 
*/

const startDirectMessage = async (targetUser) =>{
    if( !targetUser || !client?.user) return ;
    try{
//if my id is 12 ans ur id is 34 and we start a convo 
// then we  want the convo to look like 12-34 
//stream alssows channel d. to be 64 char
